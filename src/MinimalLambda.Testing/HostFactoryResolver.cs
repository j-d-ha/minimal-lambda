// This file is copied from runtime
// Source:
// https://github.com/dotnet/runtime/blob/v10.0.0/src/libraries/Microsoft.Extensions.HostFactoryResolver/src/HostFactoryResolver.cs
...
internal sealed class HostFactoryResolver
{
    ...
    private const string TimeoutEnvironmentKey =
    ...
    )
    {
        ...
        var hostFactory = ResolveHostFactory(assembly, waitTimeout);
        if (hostFactory != null)
            return args =>
            {
            ...
            IObserver<KeyValuePair<string, object?>>
    {
        ...
        public object CreateHost()
        {
            ...
            var thread = new Thread(() =>
            {
                ...
                try
                {
                    // Set the async local to the instance of the HostingListener so we can
                    // filter events that
                    ...
                    // Try to set an exception if the entry point returns gracefully, this
                    // will force
                    // build to throw
        ...
        // So don't depend on the public HostAbortedException directly. Instead, load the exception
        // type dynamically if it can
        // be found. If it can't (possibly because the app is using an older version), throw a
        // private exception with the same name.
