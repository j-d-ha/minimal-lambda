...
public interface ILambdaApplication
{
    ...
    /// <summary>
    ///     Registers a callback handler to be invoked during the AWS Lambda Function Init phase, before any handler invocation.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         Multiple handlers can be registered and execute asynchronously using <see cref="Task.WhenAll{TResult}(System.Collections.Generic.IEnumerable{System.Threading.Tasks.Task{TResult}})"/>.
    ...
    ///         and a <see cref="CancellationToken"/> to signal when the Function Init phase should be cancelled.
    ///         The Function Init phase continues only if all handlers return <c>true</c>.
    ///         If any handler returns <c>false</c>, the Function Init phase is aborted.
    ///         See <c>LambdaHostOptions</c> for timing details and configuration options.
    ///     </para>
    ///     <para>
    ///         This is useful when using AWS Lambda Snap Start to initialize resources and state
    ///         during the Function Init phase.
    ///     </para>
    ///     <para>
    ...
    /// </remarks>
    /// <param name="handler">
    ///     A <see cref="LambdaInitDelegate"/> to invoke during the Function Init phase.
    /// </param>
    /// <returns>
    ...
    /// <summary>
    ///     Registers a callback handler to be invoked during the AWS Lambda Function Shutdown phase.
    /// </summary>
    /// <remarks>
    ///     <para>
